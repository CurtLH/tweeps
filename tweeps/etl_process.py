...
def cli(batch_size):
    ...
    while True:
        ...
        if from_n - to_n >= batch_size:
            ...
            cur.execute(
                "SELECT tweet FROM tweets_raw WHERE id BETWEEN %s AND %s"
                % (start_pos + 1, end_pos)
            )
            ...
            for line in tweets:
                ...
                try:
                    cur.execute(
                        "INSERT INTO tweets (id_str, created_at, screen_name, tweet_type, text, hashtags, urls) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                        [item for item in tweet],
                    )
                    logger.info("Successfully loaded record into twitter")

                except:
                    logger.warning(
                        "Cannot load record into twitter - id_str: %s", line["id_str"]
                    )
